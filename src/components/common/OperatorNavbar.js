...
import { operatorLogout } from "@/store/operatorAuthSlice";

export default function OperatorNavbar() {
  const dispatch = useDispatch()
  const [collapse, setCollapse] = useState(true);
  const [activeLink, setActiveLink] = useState(nav_links['dashboard']) // Default active link
  const handleLogout = async () => {
    const res = await dispatch(operatorLogout())
  };
  const [isAdmin, setIsAdmin] = useState(false)
  const url = usePathname()
  useEffect(() => {
    const currentPath = getPathByLevel(url, 2);
  ...
  return (
    !isAdmin && <nav className={`${style.nav} navbar navbar-expand-lg border-bottom`}>
      <div className="container-fluid px-3 ">
        ...
        <div className={`collapse navbar-collapse ${!collapse ? "show" : ""}`}>
          ...
          <ul className="navbar-nav me-auto align-items-center gap-1 gap-lg-2">
            {urls?.map((it) => (
              <li className="nav-item" key={it?.label}>
                <Link
                  className={`${style.link} ${activeLink == it?.href ? style.active_links : ""}  nav-link d-flex align-items-center rounded-3 gap-2 p-2 ${it?.pill ? style.pill : ""
                    }`}
                  href={it?.href}
          ...
          {/* <div className="d-flex align-items-center gap-3 ms-lg-3">
            ...
            <button onClick={handleLogout} className={`${style.logout} btn btn-link text-danger p-0`}>
