// components/operator/routes/RouteCreateForm.jsx
"use client";
    ...
    <div className={styles.panel}>
      ...
      <div className="row g-3">
        <div className="col-md-6">
          ...
          <input className="form-control shadow-none outline-0" placeholder="e.g., Mumbai–Delhi Express"
            value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
        </div>
        <div className="col-md-6">
          <label className="form-label small">Total Distance (km) *</label>
          <input className="form-control shadow-none outline-0" placeholder="0"
            value={form.totalDistanceKm} onChange={(e) => setForm({ ...form, totalDistanceKm: e.target.value })} />
        </div>
        <div className="col-md-6">
          <label className="form-label small">Source City *</label>
          <input className="form-control shadow-none outline-0" placeholder="e.g., Mumbai"
            value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })} />
        </div>
        <div className="col-md-6">
          <label className="form-label small">Destination City *</label>
          <input className="form-control shadow-none outline-0" placeholder="e.g., Delhi"
        ...
        <div className="d-flex align-items-center justify-content-between mb-2">
          ...
          <button className="btn btn-light btn-sm"
            onClick={() =>
              setForm({
                ...form,
                stops: [...form.stops, { name: "", arrival: "", departure: "", distanceKm: 0 }],
              })
            }>
          ...
          <div className={styles.stopRow} key={idx}>
            <div className="row g-2">
              <div className="col-md-3">
                ...
                <input className="form-control shadow-none outline-0" placeholder="City/Stop name"
                  value={s.name} onChange={(e) => updateStop(idx, "name", e.target.value)} />
              </div>
              <div className="col-md-3">
                <TimeField
                  label="Arrival Time *"
                  value={s.arrival}
                  onChange={(norm) => updateStop(idx, "arrival", norm)}
                  stepSeconds={60} // minutes precision
                  required={true}
                />
              </div>
              <div className="col-md-3">
                <TimeField
                  ...
                  value={s.departure}
                  onChange={(norm) => updateStop(idx, "departure", norm)}
                  stepSeconds={60} 
                  required={true}
                />
              </div>
              <div className="col-md-2">
                ...
                <input className="form-control shadow-none outline-0" placeholder="0"
                  value={s.distanceKm} onChange={(e) => updateStop(idx, "distanceKm", e.target.value)} />
              </div>
              <div className="col-md-1 d-flex align-items-end">
                <button className="btn btn-outline-light text-danger w-100"
                  onClick={() =>
